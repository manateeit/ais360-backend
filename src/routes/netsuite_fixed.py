...
def make_netsuite_request(query):
    """Make a request to NetSuite SuiteQL API"""
    try:
...
def get_estimate_requests():
    ...
    try:
        # SQL query to get estimate requests with JOINs for human-readable names
        query = """
        SELECT 
            er.id,
            er.custrecord_er_job AS job_id,
            COALESCE(job.companyname, job.altname, job.entityid) AS job_name,
            er.custrecord_er_assigned_to AS assigned_to_id,
            COALESCE(emp.altname, emp.entityid) AS assigned_to_name,
            er.custrecord_er_requested_by AS requested_by_id,
            COALESCE(req_emp.altname, req_emp.entityid) AS requested_by_name,
            er.custrecord_er_bid_due_date AS bid_due_date,
            er.custrecord_er_priority AS priority_id,
            er.custrecord_er_status AS status_id,
            er.custrecord_er_estimate_due_date AS estimate_due_date,
